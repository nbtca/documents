...
Tailscale 是一个基于 [WireGuard](https://www.wireguard.com/) 的零配置虚拟组网服务，它可以让你在任何地方都能安全地访问计协内部的服务。社团自建的 Tailscale 服务（基于开源的 Headscale）提供了一个私有的、安全的虚拟组网解决方案。

### 它是怎么工作的

每台设备生成一对 WireGuard 密钥；协调服务器（我们自建的 Headscale）只负责身份认证、分发各设备的公钥和地址，不经手实际流量。设备之间优先 NAT 打洞直连，打不通时经 DERP 中转节点转发。每台设备会分到一个 `100.x.y.z` 的虚拟 IP——`100.64.0.0/10` 是保留网段，不会与宿舍、校园网的局域网地址冲突。

明白这一层，后文的配置就不神秘了：`--login-server` 是把官方客户端指向我们自己的协调服务器；「中转节点」加的是 DERP；代理软件冲突时要排除的正是 `100.64.0.0/10` 这段地址。

### 主要功能和优势

- **零配置**：装好客户端登录即用，无需配置防火墙规则或端口转发
- **身份验证**：接入 NBTCA 的账号进行身份验证，确保只有社团成员可以访问
- **安全加密**：基于 WireGuard 协议，采用现代加密算法保护通信
...
1. **远程访问社团服务器**：在家里或宿舍访问社团的文件服务器、[校园网](/concepts/campus-network-auth)内网网站等
...
使用中遇到问题，可在社团群里提问或联系管理员。

:::info 维护信息

| 维护人                                     | 时间            |
| ------------------------------------------ | --------------- |
| [@LazuliKao](https://github.com/LazuliKao) | 2025.11.3 - Now |
