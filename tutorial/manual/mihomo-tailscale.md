...
首次接入请先完成 [Tailscale 使用指南](/tutorial/manual/tailscale-usage)。本文以未使用 Tailscale 出口节点（exit node）、通过 Tailnet IP 访问设备为前提；子网路由、MagicDNS 和多网卡出口见文末。
...
服务初始化需要时间，稍后再次查看。已有账号应恢复正常状态；若提示需要登录，按 [接入指南](/tutorial/manual/tailscale-usage)完成认证。不要通过退出账号或删除状态文件来代替排障。
